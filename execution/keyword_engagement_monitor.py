...
# -*- coding: utf-8 -*-
"""
Keyword Engagement Monitor - Find prospects expressing pain points via keyword monitoring.

This is Gojiberry AI's "Engagement Signal" - monitoring posts with pain point keywords
like "struggling with outbound", "cold email not working", etc.

Pipeline:
1. Search Google for LinkedIn posts containing pain point keywords
2. Filter posts with minimum reactions (default: 50+)
3. Scrape post engagers (people who liked/commented)
...
Manual Usage:
    python3 keyword_engagement_monitor.py --keywords "struggling with outbound" --dry_run
    python3 keyword_engagement_monitor.py --config config/keyword_signals.json --list_id 480247
"""
...
def run_full_pipeline(
    ...
    print("=" * 60)
    print("KEYWORD ENGAGEMENT MONITOR")
    print("=" * 60)
    print(f"Signal Type: Keyword Engagement (Pain Points)")
    ...
    for lead in qualified_leads:
        ...
        lead["trigger_source"] = "keyword_engagement"
        lead["trigger_date"] = timestamp
        lead["trigger_url"] = post_urls[0] if post_urls else ""  # First post URL as example
    ...
    # Save intermediate results with new naming convention
    timestamp_file = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f".tmp/keyword_engagement_{timestamp_file}.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="Keyword Engagement Monitor - Find prospects expressing pain points (Gojiberry-style)"
    )
    parser.add_argument(
        "--config",
        help="Path to config JSON file (e.g., config/keyword_signals.json)"
    )
    parser.add_argument(
        "--keywords",
        help="Search keywords (overrides config)"
    )
    parser.add_argument(
        "--days_back", type=int,
    ...
    parser.add_argument(
        "--min_reactions", type=int,
        help="Minimum reactions threshold (overrides config)"
    )
    parser.add_argument(
        "--countries", nargs="+",
        help="Allowed countries (overrides config)"
    )
    parser.add_argument(
        "--list_id", type=int,
    ...
    keywords = args.keywords or config_data.get("keywords", ["ceos"])[0]
    days_back = args.days_back or config_data.get("days_back", 7)
    min_reactions = args.min_reactions or config_data.get("min_reactions", 50)
    countries = args.countries or config_data.get("countries", ["United States", "Canada"])
    list_id = args.list_id or config_data.get("list_id")
